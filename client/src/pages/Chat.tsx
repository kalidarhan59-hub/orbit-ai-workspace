    ...
    <div className="grid min-h-[640px] gap-5 xl:grid-cols-[minmax(0,1fr)_300px]">
      <section className="flex min-h-[640px] flex-col overflow-hidden rounded-2xl border border-white/[0.08] bg-[#10182a]">
        <div className="flex flex-col gap-3 border-b border-white/[0.07] p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex min-w-0 items-center gap-3"><Avatar className="size-9 border border-violet-300/20 bg-violet-400/10"><AvatarFallback className="bg-transparent text-violet-200"><Sparkles className="size-4" /></AvatarFallback></Avatar><div className="min-w-0"><p className="truncate text-sm font-medium">{selectedAgent?.name || "ORBIT Orchestrator"}</p><p className="mt-0.5 text-xs text-slate-500">{selectedAgent ? "Специализированная роль активна" : "Готов к новой задаче"}</p></div></div>
          <div className="flex items-center gap-2"><select aria-label="Выбор агента" value={agentId} onChange={(e) => setAgentId(e.target.value)} className="orbit-select h-8 w-auto max-w-40 py-0 text-xs"><option value="">ORBIT</option>{agents?.map((agent) => <option key={agent.id} value={agent.id}>{agent.name}</option>)}</select><select aria-label="Выбор модели" value={modelId} onChange={(e) => setModelId(e.target.value)} className="orbit-select h-8 w-auto max-w-44 py-0 text-xs"><option value="">Автовыбор</option>{models?.map((model) => <option key={model.id} value={model.id}>{model.id}</option>)}</select></div>
        </div>
        <ScrollArea ref={scrollRef} className="min-h-0 flex-1"><div className="mx-auto flex w-full max-w-4xl flex-col gap-5 p-4 sm:p-6">{loadingSession ? <LoadingMessages /> : messages.length ? messages.map((message) => <MessageBubble key={message.id} message={message} stream={streamingId === message.id} onStreamDone={() => setStreamingId(null)} />) : <Welcome onPrompt={(prompt, nextMode) => { setInput(prompt); setMode(nextMode); }} />}{send.isPending && <div className="flex items-center gap-3 text-sm text-slate-400"><span className="flex size-8 items-center justify-center rounded-full bg-violet-400/10"><Loader2 className="size-4 animate-spin text-violet-200" /></span>{mode === "image" ? "ORBIT создаёт изображение…" : "ORBIT обрабатывает контекст и готовит ответ…"}</div>}</div></ScrollArea>
        <div className="border-t border-white/[0.07] bg-[#0d1425] p-3 sm:p-4"><div className="mx-auto max-w-4xl"><AnimatePresence>{attachments.length > 0 && <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 5 }} className="mb-2 flex flex-wrap gap-2">{attachments.map((file) => <AttachmentChip key={file.id || file.url} file={file} onRemove={() => setAttachments((items) => items.filter((item) => item.url !== file.url))} />)}</motion.div>}</AnimatePresence><form onSubmit={(event) => { event.preventDefault(); sendMessage(input); }} className="rounded-2xl border border-white/[0.10] bg-white/[0.04] p-2 focus-within:border-cyan-300/35 focus-within:ring-2 focus-within:ring-cyan-300/10"><Textarea value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); sendMessage(input); } }} placeholder={mode === "image" ? "Опишите изображение, которое хотите получить…" : "Сообщение для ORBIT…"} className="min-h-20 resize-none border-0 bg-transparent px-2 py-2 text-sm shadow-none focus-visible:ring-0" /><div className="flex flex-wrap items-center justify-between gap-2 border-t border-white/[0.06] px-1 pt-2"><div className="flex items-center gap-1"><input ref={fileInputRef} className="hidden" type="file" accept="image/*,audio/*,video/*,.pdf,.txt,.md,.json,.csv" onChange={(e) => chooseFile(e.target.files?.[0])} /><Button type="button" variant="ghost" size="icon" disabled={upload.isPending || attachments.length >= 5} onClick={() => fileInputRef.current?.click()} aria-label="Прикрепить файл" className="size-8 text-slate-400 hover:bg-white/[0.08] hover:text-white"><Paperclip className="size-4" /></Button><Button type="button" variant="ghost" size="icon" onClick={isRecording ? stopRecording : startRecording} disabled={isTranscribing || send.isPending} aria-label={isRecording ? "Остановить запись" : "Записать голосовое сообщение"} className={cn("size-8 text-slate-400 hover:bg-white/[0.08] hover:text-white", isRecording && "bg-rose-400/15 text-rose-200 hover:bg-rose-400/20")} >{isRecording ? <Square className="size-3.5 fill-current" /> : isTranscribing ? <Loader2 className="size-4 animate-spin" /> : <Mic className="size-4" />}</Button><Button type="button" variant="ghost" onClick={() => setMode(mode === "image" ? "chat" : "image")} className={cn("h-8 gap-1.5 px-2 text-xs text-slate-400 hover:bg-white/[0.08] hover:text-white", mode === "image" && "bg-cyan-300/10 text-cyan-100 hover:bg-cyan-300/15")}><ImagePlus className="size-3.5" /> Изображение</Button></div><Button type="submit" size="sm" disabled={!input.trim() || send.isPending || isTranscribing} className="orbit-button h-8 bg-violet-500 px-3 text-white hover:bg-violet-400"><Send className="mr-1.5 size-3.5" /> Отправить</Button></div></form><p className="mt-2 flex items-center gap-1.5 px-1 text-[11px] text-slate-500">{isRecording ? "Идёт запись — нажмите квадрат, чтобы распознать и отправить голосовое сообщение." : isTranscribing ? "ORBIT распознаёт голос и автоматически отправит текст." : "Enter — отправить · Shift + Enter — новая строка · до 5 вложений"}</p></div></div>
      </section>
      <aside className="space-y-4"><section className="rounded-2xl border border-white/[0.08] bg-[#10182a] p-5"><p className="text-xs font-medium uppercase tracking-[0.16em] text-cyan-300/80">Контекст сессии</p><h2 className="mt-2 text-lg font-semibold">{selectedAgent?.name || "Универсальный помощник"}</h2><p className="mt-2 text-sm leading-6 text-slate-400">{selectedAgent?.description || "ORBIT помогает превращать запрос в связный, проверяемый результат и сохраняет историю работы."}</p><dl className="mt-5 space-y-3 text-xs"><InfoRow label="Модель" value={modelId || selectedAgent?.modelId || "Автовыбор"} /><InfoRow label="Память" value={selectedAgent?.memoryEnabled ? "Агентная" : "Только сессия"} /><InfoRow label="Режим" value={mode === "image" ? "Генерация изображения" : "Диалог"} /></dl></section><section className="rounded-2xl border border-white/[0.08] bg-white/[0.025] p-5"><p className="text-xs font-medium uppercase tracking-[0.16em] text-slate-500">Доступные файлы</p>{storedFiles?.length ? <div className="mt-4 space-y-2">{storedFiles.slice(0, 4).map((file) => <button key={file.id} onClick={() => setAttachments((items) => items.some((item) => item.id === file.id) ? items : [...items, attachmentFromFile(file)].slice(0, 5))} className="flex w-full items-center gap-2 rounded-lg px-2 py-2 text-left text-xs text-slate-400 transition-colors hover:bg-white/[0.05] hover:text-white"><FileIcon type={file.mimeType} /><span className="min-w-0 flex-1 truncate">{file.name}</span><Plus className="size-3" /></button>)}</div> : <p className="mt-3 text-sm leading-6 text-slate-500">Загруженные материалы появятся здесь и смогут быть добавлены в контекст одним нажатием.</p>}</section><section className="rounded-2xl border border-emerald-300/12 bg-emerald-300/[0.045] p-5"><CheckCircle2 className="size-4 text-emerald-300" /><p className="mt-3 text-sm font-medium text-emerald-100">Контроль сохранён</p><p className="mt-1 text-xs leading-5 text-emerald-100/60">Вложения и память изолированы вашей учётной записью. Модель не получает скрытые ключи.</p></section></aside>
    </div>
  </OrbitPage>;
}
...
function Welcome({ onPrompt }: { onPrompt: (prompt: string, mode: "chat" | "image") => void }) { return <div className="my-auto py-14 text-center"><span className="mx-auto flex size-12 items-center justify-center rounded-2xl bg-violet-400/12 text-violet-200"><Bot className="size-5" /></span><h2 className="mt-5 text-2xl font-semibold tracking-tight">Что нужно сделать?</h2><p className="mx-auto mt-3 max-w-lg text-sm leading-6 text-slate-400">Опишите задачу, прикрепите материал или надиктуйте сообщение. ORBIT сохранит историю и применит контекст выбранного агента.</p><div className="mx-auto mt-7 grid max-w-2xl gap-2 text-left sm:grid-cols-2"><Prompt title="Сравнить варианты" text="Сравни инструменты для небольшой команды и выдели ограничения." onClick={() => onPrompt("Сравни современные AI-инструменты для небольшой команды: возможности, ограничения и приватность.", "chat")} /><Prompt title="Разобрать документ" text="Прикрепите PDF или таблицу, затем попросите сделать выводы." onClick={() => onPrompt("Помоги выделить риски, решения и следующие шаги из приложенного документа.", "chat")} /><Prompt title="Создать изображение" text="Опишите идею — результат появится прямо в переписке." onClick={() => onPrompt("Создай минималистичную иллюстрацию для рабочего пространства AI-агентов: орбита, графитовый фон, violet и cyan акценты.", "image")} /><Prompt title="Сохранить предпочтение" text="Создайте агента и добавьте полезную заметку памяти." onClick={() => onPrompt("Подскажи, как настроить агента для анализа документов и сохранения ключевых предпочтений.", "chat")} /></div></div>; }
