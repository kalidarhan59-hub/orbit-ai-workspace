...
export default function VoiceChat() {
  ...
  const activeVoiceName = voices.find((voice) => voice.voiceURI === selectedVoice)?.name || "Автовыбор русского голоса";
  const quickPrompts = ["Объясни сложную тему простыми словами", "Помоги составить план", "Разбери мою идею", "Напиши короткий код"];
  ...
  return <OrbitPage eyebrow="Разговор в реальном времени" title="Voice Chat" action={<div className="flex items-center gap-2"><Button onClick={() => setSettingsOpen((open) => !open)} variant="outline" className="border-white/[0.12] bg-white/[0.03] text-slate-100 hover:bg-white/[0.08]" aria-label="Настройки голоса"><Settings2 className="size-4" /></Button><Button onClick={stopAll} variant="outline" disabled={!isActive} className="border-white/[0.12] bg-white/[0.03] text-slate-100 hover:bg-white/[0.08]"><Square className="mr-2 size-4" /> Остановить</Button></div>}>
    <div className="relative flex min-h-[calc(100vh-13rem)] flex-col overflow-hidden rounded-[2rem] border border-white/[0.10] bg-[#080d1d] px-5 py-8 shadow-[0_24px_90px_rgba(2,6,23,.45)] sm:px-10 lg:py-12">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_50%_35%,rgba(139,92,246,.16),transparent_34%),radial-gradient(circle_at_20%_80%,rgba(34,211,238,.08),transparent_30%)]" /><div className="pointer-events-none absolute -left-24 top-[-10rem] size-[28rem] rounded-full bg-violet-500/[0.12] blur-[105px]" /><div className="pointer-events-none absolute bottom-[-12rem] right-[-7rem] size-[26rem] rounded-full bg-cyan-300/[0.10] blur-[100px]" /><div className="pointer-events-none absolute inset-x-10 top-24 h-px bg-gradient-to-r from-transparent via-violet-300/20 to-transparent" />
      ...
      <div className="relative mx-auto flex w-full max-w-2xl flex-1 flex-col items-center justify-center text-center">
        <div className="relative flex items-center justify-center py-8 sm:py-12">
          <span className={cn("pointer-events-none absolute size-[18rem] rounded-full border border-violet-300/20", isActive && "animate-[spin_18s_linear_infinite] border-cyan-200/30")} />
          <span className={cn("pointer-events-none absolute size-[15rem] rounded-full border border-cyan-300/15", voiceState === "thinking" && "animate-[spin_10s_linear_infinite_reverse]")} />
        ...
        <p className="mt-8 text-xl font-semibold tracking-tight text-white sm:text-2xl">{stateCopy[voiceState].title}</p><p aria-live="polite" className="mt-3 max-w-lg text-sm leading-6 text-slate-400">{stateCopy[voiceState].description}</p>
        <div className="mt-8 flex flex-wrap justify-center gap-2"><span className="inline-flex items-center gap-2 rounded-full border border-cyan-300/15 bg-cyan-300/[0.07] px-3 py-1.5 text-xs text-cyan-100/80"><Radio className="size-3.5 text-cyan-300" /> Русская речь</span><span className="inline-flex max-w-full items-center gap-2 rounded-full border border-emerald-300/15 bg-emerald-300/[0.06] px-3 py-1.5 text-xs text-emerald-100/80"><Volume2 className="size-3.5 shrink-0 text-emerald-300" /> <span className="truncate">{activeVoiceName}</span></span><span className="inline-flex items-center gap-2 rounded-full border border-violet-300/15 bg-violet-300/[0.07] px-3 py-1.5 text-xs text-violet-100/80"><Sparkles className="size-3.5 text-violet-300" /> ORBIT Intelligence</span></div>
        <div className="mt-7 flex max-w-2xl flex-wrap justify-center gap-2">{quickPrompts.map((prompt) => <button key={prompt} type="button" onClick={() => submitTranscript(prompt)} className="rounded-full border border-white/[0.09] bg-white/[0.035] px-3.5 py-2 text-xs text-slate-400 transition hover:border-cyan-300/30 hover:bg-cyan-300/[0.07] hover:text-cyan-100 active:scale-95">«{prompt}»</button>)}</div>
      </div>
      <div className="relative mx-auto grid w-full max-w-4xl gap-4 border-t border-white/[0.08] pt-7 md:grid-cols-2"><TranscriptCard icon={<MessageSquareText className="size-4 text-cyan-200" />} title="Вы сказали" content={lastQuestion || liveText || "После нажатия на орбиту ваша фраза появится здесь."} /><TranscriptCard icon={<Volume2 className="size-4 text-emerald-200" />} title="ORBIT ответил" content={lastAnswer || "Ответ будет показан текстом и одновременно озвучен."} /></div>
    </div>
  </OrbitPage>;
}

function TranscriptCard({ icon, title, content }: { icon: React.ReactNode; title: string; content: string }) { return <section className="group rounded-2xl border border-white/[0.09] bg-white/[0.035] p-4 text-left shadow-[0_12px_35px_rgba(2,6,23,.2)] backdrop-blur-xl transition hover:border-white/[0.16]"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">{icon}{title}</div><p className="mt-3 min-h-12 line-clamp-4 text-sm leading-6 text-slate-300">{content}</p></section>; }
